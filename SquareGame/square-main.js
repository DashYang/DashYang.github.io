    ...
    if (typeof resetGame === 'function') {
        ...
        var _waiter = setInterval(function(){
            ...
            if (typeof resetGame === 'function') {
                ...
                try { resetGame(); } catch(e) { console.log('resetGame error', e); }
                try { popTutorial(); } catch(e) {}
            } else if (_attempts > 60) { // ~3s
                clearInterval(_waiter);
                console.log('resetGame not available after waiting; skipping initial reset');
            }
        }, 50);
    }
};
...
G.F.mainAI = function () {
    try {
        ...
        try {
            if (G.O && G.O.explosion && typeof G.O.explosion.AI === 'function') G.O.explosion.AI();
        } catch(e) {}
        if(timer <= 0 && gamestate == "on") {
            ...
            try { squareManage(); } catch(e) { console.log('squareManage error', e); }
            // update dashboard via centralized updater
            try { if (typeof updateDashboard === 'function') updateDashboard(); } catch(e) { console.log('updateDashboard error', e); }
        }

        try {
            if(G.O.viewport && G.O.viewport.tagContainsMouseClick && G.O.viewport.tagContainsMouseClick() && gamestate == "off" && isTouched == false){
                resetGame();
            }
        } catch(e) {}

        try {
            if(G.O.tutorial && G.O.tutorial.tagContainsMouseClick && G.O.tutorial.tagContainsMouseClick() && isTouched == false) {
                if( gamestate == "on")
                    popTutorial();
        ...
        try { gamestate = 'off'; } catch(e) {}
        try { showEndScreen(score); } catch(e) {}
    }
};
